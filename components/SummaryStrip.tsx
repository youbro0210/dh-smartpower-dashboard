import { DeviceEvaluated, Bridge, ThresholdConfig, DeviceStatus } from "@/lib/types";
...
export default function SummaryStrip({
  ...
  const temperatures = online.map((d) => d.temperature);
  ...
  return (
    <div className="summary-strip">
      ...
      <div className="summary-item total">
        <span className="n">{avgTemp.toFixed(1)}℃</span>
        <span className="l">평균온도 (최고 {maxTemp.toFixed(1)}℃)</span>
      </div>
      ...
      <div className="summary-item normal">
        <span className="n">{bridgesOnline}/{bridges.length}</span>
        <span className="l">브릿지 온라인</span>
      </div>
    </div>
  );
}
