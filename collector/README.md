...
브로커 주소 · 계정 · TLS · 토픽 접두사는 **설정 › 데이터 연결** 에서 바꿉니다.
저장하면 수집기가 통지를 받아 곧바로 다시 접속합니다. 같은 화면에서 수집기가
살아 있는지, 브로커에 붙어 있는지, 마지막 수신이 언제인지 확인합니다.

**설정 › 수집 내역** 은 장비가 실제로 보낸 메시지를 원문 그대로 보여줍니다.
규격이 어긋났을 때 장비 쪽 문제인지 서버 쪽 문제인지 여기서 가려집니다.
JSON 이 깨진 건, items 가 빈 건, 미등록 설비가 올린 건이 각각 구분되어 남습니다.
...
명령 경로는 `fake_device.py` 로 확인합니다. 명령을 받아 응답을 돌려주는
가짜 장치입니다.
